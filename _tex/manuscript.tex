...
a patient in t-stage \(T_i\) and with involvement \(\xi_i\) to the power
of \(N_i\), corresponding to the number of observations of the specific
diagnosis \(i\) in the dataset \(\mathbf{D}\). By multiplying over all
diagnoses \(i\) we get the likelihood of observing dataset
\(\mathbf{D}\) given the previously intoduced model with parameters
\(\boldsymbol{\theta}\)
...
Assume we have a dataset \(\mathbf{D}\) comprising the number of
patients \(N_{is}\) diagnosed with LNL involvement state \(i\) and
having a primary tutor in subsite \(s\). Let us further assume that we
want to describe this dataset using a mixture of \(M\) HMMs, each with a
different set of model parameters \(\boldsymbol{\theta_m}\).

As the generative model of the data, we assume that a patient with
subsite \(s\) is generated with probability \(\pi_m^s\) from model
\(m\),which satisfies the constraints \(\sum_{m = 1}^{M}\pi_m^s = 1\)
and \(\pi_m^s \geq 0\) for all \(m\) and all subsites \(s\).
...
In equation~\ref{eq-mixture-distribution} we compute the probability of
each state \(\boldsymbol{\xi}_i\) under model \(m\) similarly to
equation~\ref{eq-likelihood-HMM}. However, we weight each model with the
mixing parameter \(\pi_m^s\). Since the mixing parameters are subsite
dependent, we differ between each subsite.
...
\(\boldsymbol{\epsilon}\), which represent the probability that each
observation \(\mathbf{X} = \boldsymbol{\xi}_i\) originates from model
\(m\). Given the current estimates of \(\boldsymbol{\theta}\) and
\(\boldsymbol{\pi}\), the expectation value \(\gamma(\epsilon_m^i)\) is
calculated as:
...
Here, the mixing parameter \(\pi_m^s\) depends on the subsite of
diagnosis, \(Z_i\), as denoted by the superscript \(s\).

\paragraph{Maximization Step}\label{maximization-step}

In the M-step, the model parameters are updated by maximizing the
following likelihood, based on the expectations computed in the E-step:
...
\begin{equation}\phantomsection\label{eq-mixture-llh}{
