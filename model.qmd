...
from scripts.marginalize_midext import compute_possible_observed_prevalences, SummedBetaPosterior
...
for i, t_stage in enumerate(["early", "late"]):
  ...
  for j, midext in enumerate([False, True]):
    ...
    for lnl, color in zip(
      ...
      num_success, num_total = compute_possible_observed_prevalences(data, scenario).T
      content.append(
        SummedBetaPosterior(
          num_success=num_success,
          num_total=num_total,
          kwargs={"label": None, "linestyle": "--"},
        )
      )
    draw(
      axes=axes[i,j],
...
from scripts.marginalize_midext import (
  compute_possible_observed_prevalences,
  SummedBetaPosterior,
)
...
from lyscripts.plot.utils import Histogram, BetaPosterior, draw, COLORS, split_legends
from scripts import paths, shared
from scripts.marginalize_midext import compute_match_total_combinations, SummedBetaPosterior
...
for t_stage in ["early", "late"]:
  ...
  num_success, num_total = compute_match_total_combinations(
    base_match=beta_post.num_success,
    base_total=beta_post.num_total,
    unknown_match=(has_unknown_midext & has_t_stage).sum(),
    unknown_total=(has_unknown_midext & has_t_stage).sum(),
  ).T
  content.append(
    SummedBetaPosterior(
      num_success=num_success,
