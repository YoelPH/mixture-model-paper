...
plt.rcParams.update(shared.get_fontsizes())
plt.rcParams.update(shared.get_figsizes(width=17./2., aspect_ratio=2.))
...
_v, bins, patch = Histogram.from_hdf5(
  filename=paths.model_dir / "full" / "risks.hdf5",
  dataname=scenario.md5_hash("risks"),
).draw(ax, hist=defaults)

patches["early T-cat."].append(*patch)
patches["lateralized"].append(*patch)
patches["ipsi N0"].append(*patch)
...
patches["advanced"].append(*patch)
patches["lateralized"].append(*patch)
patches["≥ ipsi LNL III"].append(*patch)
...
patches["early T-cat."].append(*patch)
patches["midline ext."].append(*patch)
patches["ipsi N0"].append(*patch)
...
for label in patches:
  handle = tuple(patches[label])
  patches[label] = handle

ax.legend(
  handles=patches.values(),
  labels=patches.keys(),
