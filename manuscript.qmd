...
$$
\ell = \sum_{t=0}^{t_\text{max}} \left[ \boldsymbol{\xi}_0 \cdot \mathbf{A} \cdot \mathbf{B} \right]_i \cdot P \left( t \mid \text{T}x \right)
$$ {#eq-single-patient-llh}

Note that it is also possible to account for missing involvement information: If a diagnosis (like fine needle aspiration (FNA)) is only available for a subset of all LNLs, we can sum over all those possible complete observed states $\boldsymbol{\zeta}_j$ that match the provided diagnosis.
...
$$
\boldsymbol{\theta} = \left( \left\{ b_v \right\}, \left\{ t_{vr} \right\}, \left\{p_\text{T}x \right\} \right) \quad \text{with} \quad \genfrac{}{}{0pt}{2}{v\leq V}{r\in\operatorname{pa}(v)}
$$
...
{{< embed model.qmd#fig-model-midext-evo >}}
...
{{< embed model.qmd#fig-model-burnin-history >}}
...
for col, model in enumerate(["contra", "bilateral", "midline"]):
  ...
  for row, t_stage in enumerate(["early", "late"]):
    ...
    for scenario in scenarios:
      ...
      if model == "contra":
        scenario.side = "contra"
        scenario.is_uni = True
      else:
        scenario.side = "ipsi"
