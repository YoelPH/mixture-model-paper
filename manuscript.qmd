...
raw = pd.read_csv(paths.data, header=[0,1,2])
data = raw.copy()
...
To the best of our knowledge, this data still remains the only publicly available data on patterns of lymphatic progression containing this level of detail. It allows us to visualize these patterns in so-called *upset plots* [@lex_upset_2014] as shown in @fig-data-upset.
...
scenarios_config = utils.load_yaml_params("scenarios/base.yaml")
...
for col, model in enumerate(["contra", "bilateral", "midline"]):
  ...
  for row, t_stage in enumerate(["early", "late"]):
    ...
    for scenario in scenarios:
      ...
      content += [
        Histogram.from_hdf5(
          filename=paths.get_filename(model=model, kind="prevalences"),
