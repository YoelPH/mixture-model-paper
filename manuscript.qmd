...
col_map = {
  ("patient", "#", "institution"): ("Institution", lambda x: None),
  ("patient", "#", "age"): ("Age", np.mean),
  ("tumor", "1", "t_stage"): ("T-category", np.mean),
}
reduced = raw[col_map.keys()]
reduced.columns = reduced.columns.get_level_values(2)
renamed = (
  reduced
  .rename(columns={tpl[-1]: val[0] for tpl, val in col_map.items()})
  .convert_dtypes()
)
grouped = renamed.groupby(by="Institution")
aggregated = grouped.agg({
  name: func for name, func in col_map.values()
  if name != "Institution"
})
```
...
for i, institution in enumerate([
  ...
  is_from_institution = data["patient", "#", "institution"] == institution
  inst_data = data.loc[is_from_institution]
  locations = inst_data["tumor", "1", "location"].unique()
  locations = locations[~pd.isna(locations)]

  if len(locations) == 1:
    locations_str = str(locations[0])
  else:
    locations_str = ", ".join(locations[:-1]) + ", and " + str(locations[-1])

  print(f"{i}. {len(inst_data)} patients with newly diagnosed HNSCC in the {locations_str}, extracted at the {institution}.")
```
