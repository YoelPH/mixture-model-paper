...
To account for the increased prevalence of involvement on the contralateral side when the tumor is not clearly lateralized anymore, we also model the tumor's extension over the mid-sagittal line as a binary random variable. It starts lateralized and every time-step there is a finite probability $p_\epsilon$ that the tumor grows over the symmetry plane of the patient.
...
As with the state of the lymphatic network, we can also define a small transition matrix $\mathbf{A}_\epsilon$ that evolves the lateralized starting state $\boldsymbol{\epsilon}[0] = \begin{pmatrix} 1 & 0 \end{pmatrix}$:

$$
\boldsymbol{\epsilon}[t] = \boldsymbol{\epsilon}[0] \cdot \mathbf{A}_\epsilon^t = \begin{pmatrix} 1 & 0 \end{pmatrix} \cdot \begin{pmatrix} 1 - \epsilon & \epsilon \\ 0 & 1 \end{pmatrix}^t = \begin{pmatrix} (1 - \epsilon)^t & 1 - (1 - \epsilon)^t \end{pmatrix}
$$

We can again collect the entire evolution in a matrix $\boldsymbol{\Lambda}_\epsilon$ that stores the probabilies $P(\boldsymbol{\epsilon} \mid t)$ for each time-step $t$:

$$
\boldsymbol{\Lambda}_\epsilon = P(\boldsymbol{\epsilon} \mid t) = \begin{pmatrix}
\boldsymbol{\epsilon}[0] \cdot \mathbf{A}_\epsilon^0 \\
\boldsymbol{\epsilon}[1] \cdot \mathbf{A}_\epsilon^1 \\
\vdots \\
\boldsymbol{\epsilon}[t] \cdot \mathbf{A}_\epsilon^t \\
\end{pmatrix}
$$

This quantity has $t$ rows, one for each time-step, and two columns: one for the decreasing probability of a lateralized tumor and one for the increasing probability of a tumor that has crossed the mid-sagittal line.

```{python}
#| echo: false
#| label: fig-midext-evo
#| fig-cap: The matrix $\boldsymbol{\Lambda}_\epsilon$ shown as heatmap of probabilities.
...
cmap = LinearSegmentedColormap.from_list("redusz", [COLORS["gray"], COLORS["red"]])
im = ax.imshow(model.midext_evo().T, cmap=cmap)

# create an Axes on the right side of ax. The width of cax will be 5%
# of ax and the padding between cax and ax will be fixed at 0.05 inch.
divider = make_axes_locatable(ax);
cax = divider.append_axes("right", size="5%", pad=0.05);
plt.colorbar(im, cax=cax, label="Prob. of midext");

ax.set_xticks(np.arange(model.max_time + 1));
ax.set_xlabel("time-step $t$");

ax.set_yticks([0., 1.], labels=["lat.", "ext."])
```
...
from lyscripts.plot.utils import draw, BetaPosterior, Histogram, COLOR_CYCLE
...
early_midext_prob = model.get_distribution("early").pmf @ model.midext_evo()[:,1]
late_midext_prob = model.get_distribution("late").pmf @ model.midext_evo()[:,1]
...
The predicted prevalence of midline extension for early T-category tumors is `{python} f"{early_midext_prob:.2%}"`. For a late T-category tumor, it is `{python} f"{late_midext_prob:.2%}"`.

In the data, the prevalence of midline extension for early T-category tumors is `{python} f"{early_midext_data:.2%}"`. For late T-category tumors it is `{python} f"{late_midext_data:.2%}"`.
