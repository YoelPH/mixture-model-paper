...
model_dir = Path("models/midline")
filename = model_dir / "prevalences.hdf5"

params = utils.load_yaml_params(model_dir / "params.yaml")
model = utils.create_model(params)
scenarios_config = utils.load_yaml_params("scenarios.yaml")
scenarios = Scenario.list_from_params(
  params=scenarios_config,
...
from lyscripts.utils import create_model

with open("models/midline/params.yaml", mode="r") as file:
  config = yaml.safe_load(file)

model = create_model(config)
backend = HDFBackend("models/midline/samples.hdf5", read_only=True)
chain = backend.get_chain(flat=True)

param_names = list(model.get_params().keys())
param_means = chain.mean(axis=0)
param_stds = chain.std(axis=0)
model.set_params(*param_means)
