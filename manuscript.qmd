...
$$
P \left( \mathbf{D} \mid \boldsymbol{\theta}, \boldsymbol{\pi}\right) = \prod_i \left[ \sum_{t=0}^{t_\text{max}} P_{T_i}(t) \left( \boldsymbol{\pi} \cdot \mathbf{A}^t \right)_i \right]^{N_{i}}
$$ {#eq-likelihood-HMM}
...
Simlarly to the three component model, we can analyze the convergence over the iterations of the EM-algorithm. In @fig-fig-convergence4 we can see that in this more complex model, the likelihood space becomes more complex as at around 200 iterations, the negative log-likelihood starts to increase faster again.
